...
solutions = [
    ("Step 1", "HW 설계 검증 Agent", "3계층 검증 (인프라/모터논리/페리페럴) → 리포트"),
    ("Step 2", "CubeMX 자동화",     ".ioc 수정 → CubeMX CLI → 스니펫 주입 → ZIP"),
    ("Step 3", "알고리즘 통합",      "오픈소스 FOC 코드를 USER CODE 영역에 자동 삽입"),
]
...
add_text(s, "Step 2는 LLM 없이 동작.  Step 1 · 3 모델 합산 ~42GB → DGX Spark 128GB 내 동시 상주 가능 (큰 여유)",
...
models = [
    ("Step 1 — 검증", "Gemma 4 31B Dense (Q4_K_M, ~20GB)"),
    ("Step 3 — 통합", "Gemma 4 26B MoE (Q8, ~22GB, Active ~4B)"),
    ("임베딩",         "BAAI/bge-m3 (한국어+영어 지원)"),
    ("동시 로드",      "~42GB 합산 → 128GB 내 큰 여유 ✓"),
]
...
phases = [
    ...
    ("Phase 2", "Step 1\nHW 검증 Agent", "3~4주", TEAL,  "3계층 규칙 엔진\nRAG 구성\nGemma 4 31B 연결"),
    ("Phase 3", "Step 2\nCubeMX 자동화", "2~3주", GREEN, ".ioc 템플릿 수정\nCubeMX CLI 연동\n스니펫 주입"),
...
add_text(s, "① ST 공식 문서 수집 (✅ 14건 완료)", 0.4, 1.85, 6.0, 0.38, size=14, bold=True, color=BLUE)
tasks1 = [
    "rm0440 레퍼런스 매뉴얼 (39MB)",
    "stm32g474re 데이터시트 + AN5306 OPAMP + AN5346 ADC",
    "AN5348 FDCAN + AN3070 RS-485 + AN5031 HW가이드",
    "EVSPIN32G4-DUAL 회로도/매뉴얼/BOM/제조데이터",
    "um3027/um3016 MCSDK Workbench/Profiler",
]
for i, t in enumerate(tasks1):
    add_text(s, f"    {'▸' if i>0 else '📌'} {t}", 0.4, 2.28+i*0.38, 6.0, 0.36, size=12, color=BLACK)

add_text(s, "② 오픈소스 프로젝트 (8개 등록)", 0.4, 4.25, 6.0, 0.38, size=14, bold=True, color=BLUE)
modules = [
    "✅ flatmcu (73파일) — KiCad 회로도",
    "✅ STM32CubeG4 (3,063파일) — HAL 예제",
    "⬜ Arduino-FOC / stm32-esc / moteus",
    "⬜ MESC / VESC / ODrive (submodule 미초기화)",
]
for i, m in enumerate(modules):
    add_text(s, f"    {m}", 0.4, 4.68+i*0.38, 6.0, 0.36, size=12, color=GREEN if m.startswith("✅") else ORANGE)

# 우측
add_text(s, "③ Golden Module (작성 현황)", 6.8, 1.85, 6.0, 0.38, size=14, bold=True, color=ORANGE)
new_mods = [
    ("✅", "bldc_6step_hall.c", "6-step Hall + BRK 보호"),
    ("✅", "dc_motor_pid.c",    "H-bridge PWM + PID"),
    ("✅", "fdcan_motor_cmd.c", "FDCAN 커맨드 파싱"),
    ("✅", "multi_axis_sync.c", "TIM1/TIM8/TIM20 동기화"),
    ("⬜", "foc_clarke/park/...","오픈소스에서 가공 예정"),
]
for i, (status, fname, desc) in enumerate(new_mods):
    bg = LIGHTGREEN if status=="✅" else LIGHTORANGE
    add_rect(s, 6.8, 2.28+i*0.58, 6.0, 0.52, fill=bg)
    add_text(s, f"{status} {fname}", 6.95, 2.3+i*0.58, 2.8, 0.28, size=12, bold=True, color=GREEN if status=="✅" else ORANGE)
...
add_text(s, "RAG 지식베이스\n~2,600 청크", 6.9, 5.13, 2.7, 0.6, size=12.5, bold=True, color=BLUE, align=PP_ALIGN.CENTER)
add_rect(s, 9.95, 5.08, 2.85, 0.7, fill=LIGHTGREEN)
add_text(s, "Fine-tuning 데이터셋\n500~1,000쌍 (자동생성)", 10.05, 5.13, 2.7, 0.6, size=12.5, bold=True, color=GREEN, align=PP_ALIGN.CENTER)

# ══════════════════════════════════════════════════════════════
# 슬라이드 8 — Phase 2: HW 설계 검증 Agent
# ══════════════════════════════════════════════════════════════
s = prs.slides.add_slide(blank_layout)
slide_header(s, "Phase 2 — Step 1 HW 설계 검증 Agent", "기간: 3~4주")
footer(s, 8)
...
checks = [
    ("①", "VDD/VCAP",     "전원 핀 누락 확인 (ERROR)"),
    ("①", "BOOT0/NRST",   "플로팅 방지, 리셋 회로 (ERROR)"),
    ("①", "SWD 디버그",    "SWDIO/SWCLK 연결 확인 (WARN)"),
    ("②", "상보 PWM 쌍",  "TIM1_CH1 + CH1N 동일 타이머 (ERROR)"),
    ("②", "엔코더 타이머", "Encoder Mode 전용 TIM 확인 (ERROR)"),
    ("③", "OPAMP/ADC 충돌","내부 연결 충돌 감지 (ERROR)"),
]
...
# ══════════════════════════════════════════════════════════════
s = prs.slides.add_slide(blank_layout)
slide_header(s, "Phase 4 — Step 3  알고리즘 통합 Agent", "기간: 4~6주  |  Gemma 4 26B MoE")
footer(s, 10)
...
rag_items = [
    ("검색 대상",   "오픈소스 FOC 코드 + golden_modules/\n(8개 오픈소스 프로젝트 기반)"),
...
ft_rows = [
    ("검증 모델", "Gemma 4 31B Dense", "QLoRA", "~60~70GB", "3계층 리포트 포맷 통일"),
    ("통합 모델", "Gemma 4 26B MoE", "QLoRA", "~40~50GB", "USER CODE 삽입 정확도"),
]
...
strategies = [
    ("1순위", "오픈소스 FOC 코드 + CubeMX 생성 코드 조합",  "즉시 시작 가능, ~20~30개"),
    ("2순위", "오픈소스 회로도 기반 검증 에러 자동 생성",   "~30~50개 추가 확보"),
    ("3순위", "Golden Module 조합 자동 생성",              "컴파일 검증 후 확장"),
...
add_text(s, "💡  DGX Spark 128GB → 31B Dense / 26B MoE QLoRA 파인튜닝 모두 가능",
...
# ══════════════════════════════════════════════════════════════
s = prs.slides.add_slide(blank_layout)
slide_header(s, "학습 데이터 전략 — BLDC 제어 알고리즘 구조", "오픈소스 FOC 코드 기반 → Golden Module 가공·등록")
footer(s, 12)
...
gm_rows = [
    ("bldc_6step_hall.c",   "✅", "6-step Hall + BRK 보호"),
    ("dc_motor_pid.c",      "✅", "DC H-bridge PID (완성)"),
    ("fdcan_motor_cmd.c",   "✅", "FDCAN 커맨드 파싱 (완성)"),
    ("multi_axis_sync.c",   "✅", "TIM 동기화 (완성)"),
    ("foc_clarke.c",        "⬜", "Clarke (오픈소스 가공 예정)"),
    ("foc_park.c",          "⬜", "Park + CORDIC (가공 예정)"),
    ("foc_inv_park.c",      "⬜", "역 Park (가공 예정)"),
    ("foc_svpwm.c",         "⬜", "SVPWM (가공 예정)"),
    ("foc_current_pi.c",    "⬜", "PI 제어기 (가공 예정)"),
    ("foc_speed_pi.c",      "⬜", "속도 PI (가공 예정)"),
    ("foc_angle_encoder.c", "⬜", "엔코더 (가공 예정)"),
    ("foc_angle_hall.c",    "⬜", "Hall 각도 (가공 예정)"),
    ("foc_angle_smo.c",     "⬜", "SMO 옵저버 (가공 예정)"),
    ("foc_current_sense.c", "⬜", "전류감지 (가공 예정)"),
    ("foc_position_pi.c",   "⬜", "위치 PI (가공 예정)"),
]
...
step_rows = [
    ...
    ("Step 3", "알고리즘\n통합", "Gemma 4 26B MoE", "Q8", "~22GB",
     "Active ~4B → 빠른 추론, USER CODE 삽입 최적화"),
...
load_rows = [
    ("권장",       "Gemma 4 31B Q4", "Gemma 4 26B MoE Q8", "~42GB",  "✓ 큰 여유"),
    ("최고 품질",  "Gemma 4 31B Q8", "Gemma 4 26B MoE Q8", "~58GB", "✓ 여유"),
    ("경량",       "Gemma 4 26B MoE Q4", "Gemma 4 26B MoE Q4", "~36GB","✓ 큰 여유"),
]
...
ft_items = [
    ("Gemma 4 31B Dense QLoRA",  "~60~70GB"),
    ("Gemma 4 26B MoE QLoRA",   "~40~50GB"),
]
...
s3_steps = [
    ...
    "4.  [코드 생성 확정] 클릭 → Gemma 4 26B MoE 실행",
...
selected = [
    ("✅", "bldc_6step_hall.c",   "6-step Hall + BRK"),
    ("✅", "fdcan_motor_cmd.c",   "FDCAN 핸들러"),
    ("⬜", "foc_clarke.c",        "Clarke 변환 (가공 예정)"),
    ("⬜", "foc_park.c",          "Park 변환 (가공 예정)"),
    ("⬜", "foc_inv_park.c",      "역 Park (가공 예정)"),
    ("⬜", "foc_svpwm.c",         "SVPWM (가공 예정)"),
    ("⬜", "foc_current_pi.c",    "PI 제어기 (가공 예정)"),
    ("⬜", "foc_angle_encoder.c", "엔코더 (가공 예정)"),
    ("⬜", "foc_current_sense.c", "전류 감지 (가공 예정)"),
]
...
_MODS = [
    ("bldc_6step_hall.c", True,  "6-step Hall + BRK"),
    ("fdcan_motor_cmd.c", True,  "FDCAN 핸들러"),
    ("foc_clarke.c",      False, "Clarke (오픈소스 가공 예정)"),
    ("foc_park.c",        False, "Park (가공 예정)"),
    ("foc_svpwm.c",       False, "SVPWM (가공 예정)"),
    ("foc_current_pi.c",  False, "PI 제어 (가공 예정)"),
]
...
add_text(s, "코드 생성 확정  (Gemma 4 26B MoE 실행)", 0.5, S3Y+1.64, 5.3, 0.24, size=10, bold=True, color=WHITE, align=PP_ALIGN.CENTER)
...
src3 = [
    ("① Golden Module 소스 코드", BLUE,
     ["✅ bldc_6step_hall / dc_motor_pid (완성)",
      "✅ fdcan_motor_cmd / multi_axis_sync (완성)",
      "⬜ foc_clarke / foc_park / foc_svpwm (오픈소스 가공 예정)",
      "⬜ foc_current_pi / foc_angle_encoder (가공 예정)",
      "⬜ foc_current_sense (flatmcu 참조)",
      "소스: Arduino-FOC, MESC, VESC, moteus, ODrive",
      "→ 향후 사내 코드 확보 시 교체 예정"],
...
slide_header(s, "B-2  Step 1 모델 — QLoRA Fine-tuning",
             "베이스: Gemma 4 31B Dense  |  목적: 3계층 검증 리포트 포맷 통일")
footer(s, 30)
...
slide_header(s, "B-3  Step 3 모델 — QLoRA Fine-tuning & Ollama 배포",
             "베이스: Gemma 4 26B MoE  |  목적: USER CODE 통합 코드 생성")
footer(s, 31)
...
# ── 저장 ───────────────────────────────────────────────────
import os
output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "STM32G4_Agent_Plan.pptx")
