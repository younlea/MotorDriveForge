...
effects = ["핀 오류 사전 차단", "HAL 코드 생성 시간 90% 단축", "알고리즘 통합 자동화", "사내 코딩 스타일 일관성 유지"]
for i, e in enumerate(effects):
    lx = 0.4 + i * 3.1
    add_rect(s, lx, 5.45, 2.9, 0.6, fill=LIGHTBLUE)
    add_text(s, e, lx+0.1, 5.5, 2.7, 0.5, size=12.5, bold=True, color=BLUE, align=PP_ALIGN.CENTER)
...
models = [
    ("Step 1 — 검증", "Qwen2.5-72B-Instruct (Q4_K_M, ~40GB)"),
    ("Step 3 — 통합", "Qwen2.5-Coder-32B-Instruct (Q8, ~32GB)"),
...
impl_items = [
    ...
    ("LLM 연결",   "Qwen2.5-72B: 검증 결과 자연어 설명 + JSON 구조화 출력"),
...
# ══════════════════════════════════════════════════════════════
s = prs.slides.add_slide(blank_layout)
slide_header(s, "Phase 4 — Step 3  알고리즘 통합 Agent", "기간: 4~6주  |  Qwen2.5-Coder-32B")
footer(s, 10)
...
ft_rows = [
    ("검증 모델", "Qwen2.5-72B", "QLoRA", "~60~80GB", "검증 리포트 포맷 통일"),
    ("통합 모델", "Qwen2.5-Coder-32B", "QLoRA", "~30~40GB", "사내 코딩 스타일 적용"),
]
...
for i, (fname, status, desc) in enumerate(gm_rows):
    ...
    add_rect(s, 7.2, 1.85+i*0.38, 5.8, 0.35, fill=bg)
    add_text(s, status, 7.3,  1.87+i*0.38, 0.3,  0.3, size=11)
    add_text(s, fname,  7.65, 1.87+i*0.38, 2.4,  0.3, size=10.5, bold=True, color=GREEN if status=="✅" else ORANGE)
    add_text(s, desc,   10.15,1.87+i*0.38, 2.7,  0.3, size=10.5, color=GRAY)
...
step_rows = [
    ("Step 1", "핀 검증\n(추론)", "Qwen2.5-72B-Instruct", "Q4_K_M", "~40GB",
     "한국어+영어 최상급, JSON 구조화 출력 우수"),
    ("Step 2", "CubeMX\n자동화", "LLM 불필요", "—", "0GB",
     "결정론적 코드 생성 — CubeMX CLI 직접 사용"),
    ("Step 3", "알고리즘\n통합", "Qwen2.5-Coder-32B-Instruct", "Q8", "~32GB",
...
load_rows = [
    ("권장",       "Qwen2.5-72B Q4", "Qwen2.5-Coder-32B Q8", "~72GB",  "✓ 여유"),
    ("최고 품질",  "Qwen2.5-72B Q8", "Qwen2.5-Coder-32B Q8", "~104GB", "✓ 가능"),
    ("빠른 응답",  "Qwen2.5-72B Q4", "DeepSeek-Coder-V2-Lite", "~56GB","✓ 여유"),
]
...
ft_items = [
    ("Qwen2.5-72B QLoRA",       "~60~80GB"),
    ("Qwen2.5-Coder-32B QLoRA", "~30~40GB"),
]
...
s3_steps = [
    ...
    "4.  [코드 생성 확정] 클릭 → Qwen2.5-Coder-32B 실행",
...
add_text(s, "코드 생성 확정  (Qwen2.5-Coder-32B 실행)", 0.5, S3Y+1.64, 5.3, 0.24, size=10, bold=True, color=WHITE, align=PP_ALIGN.CENTER)
...
for i, (title, desc, color) in enumerate(effects_data):
    ...
    add_text(s, title, lx+0.1, 1.92, 2.7, 0.55, size=13, bold=True, color=WHITE, align=PP_ALIGN.CENTER)
    add_rect(s, lx, 2.52, 2.9, 0.68, fill=RGBColor(0xEE,0xEE,0xEE))
    add_text(s, desc, lx+0.1, 2.56, 2.7, 0.6, size=12, color=BLACK, align=PP_ALIGN.CENTER)

# 다음 단계
add_text(s, "다음 단계 (즉시 시작 가능한 항목)", 0.4, 3.4, 12.0, 0.38, size=15, bold=True, color=GREEN)
next_steps = [
    ("즉시", "기존 FOC 코드 목록 확정 + Golden Module 태깅 시작",   GREEN),
    ("즉시", "STM32G474 CubeMX XML 파싱 스크립트 작성",             GREEN),
    ("1주",  "핀 JSON → .ioc 자동 생성기 구현",                    BLUE),
    ("2~3주","FDCAN 핸들러 / 6-step 모듈 신규 작성",               BLUE),
    ("4~6주","Ollama 세팅 + RAG 파이프라인 구성",                  TEAL),
    ("8주~", "Fine-tuning 데이터 수집 및 학습 계획 수립",           ORANGE),
]
...
src_data = [
    ...
    ("③ Reference Manual RM0440 (선택 챕터)", GREEN,
     "Ch.24~25 Advanced/GP Timers (~180p)\nCh.21 ADC (~80p)  Ch.44 FDCAN (~60p)\nCh.17 CORDIC, Ch.18 FMAC (~40p)",
     "청킹: 레지스터 설명 → 레지스터 단위\n기능 설명 → 512토큰 / 64 overlap",
     "~1,200 청크", "벡터 임베딩", LIGHTGREEN),
    ("④ Errata Sheet (ES0430)", ORANGE,
     "알려진 핀/주변장치 버그 전체\n약 40페이지 — 전체 수집",
     "청킹: 이슈 항목 단위 (1 이슈 = 1 청크)",
     "~50 청크", "벡터 임베딩", LIGHTORANGE),
]
...
src3 = [
    ("① Golden Module 소스 코드", BLUE,
     ...
     "함수 단위 청킹 (AST 파싱)\n메타데이터: {module, func, g4_accel}\n예상: ~150 청크", LIGHTBLUE),
    ("② STM32CubeG4 HAL 드라이버 소스", TEAL,
...
slide_header(s, "B-2  Step 1 모델 — QLoRA Fine-tuning",
             "베이스: Qwen2.5-72B-Instruct  |  목적: 검증 리포트 포맷 통일")
footer(s, 30)
...
hp_rows = [
    ...
    ("Target modules",           "q/k/v/o/gate/up/down", "Qwen2.5 전체 레이어"),
...
model, tokenizer = FastLanguageModel.from_pretrained(
    model_name = "/workspace/models/base/Qwen2.5-72B",
    max_seq_length = 4096,
    load_in_4bit   = True,
)
...
slide_header(s, "B-3  Step 3 모델 — QLoRA Fine-tuning & Ollama 배포",
             "베이스: Qwen2.5-Coder-32B-Instruct  |  목적: USER CODE 통합 코드 생성")
footer(s, 31)
...
model, tokenizer = FastLanguageModel.from_pretrained(
    "/workspace/models/base/Qwen2.5-Coder-32B",
    load_in_4bit=True
)
model.load_adapter(
...
arch_txt = ("[브라우저]  →  Nginx :80/443  →  React :3000  +  FastAPI :8000\n"
            "FastAPI  →  Ollama :11434 (Qwen2.5-72B · Qwen2.5-Coder-32B)\n"
